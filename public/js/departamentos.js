$(document).ready(function() {
    ...
    $('.create-departamento-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#' + modalId).modal('hide');
                updateDepartamentoTable(response.data); // Actualiza la tabla con la respuesta
                $('#success-message').html('Departamento creado exitosamente').show(); // Muestra el mensaje de éxito
    ...
    $(document).on('click', '.btn-primary[data-toggle="modal"]', function() {
        ...
        let departamentosId = $(this).closest('tr').data('id');
        
        // Seleccionar la fila correspondiente usando el ID
        let row = $('tr[data-id="' + departamentosId + '"]');
        
        if (row.length) {
            // Obtener los datos del usuario desde la fila
            let departamentoId = row.find('td:nth-child(1)').text();
            let nombre = row.find('td:nth-child(2)').text();
            
            // Establecer los valores en el modal
            $('#editModal input[name="departamentoId"]').val(departamentoId); // Establecer el ID del usuario
            $('#editModal input[name="nombre"]').val(nombre);
            
            // Cambiar la acción del formulario para que apunte a la URL correcta
            $('#editModal .update-departamento-form').attr('action', '/departamentos/' + departamentoId); 
    ...
    $('.update-departamento-form').on('submit', function(e) {
        e.preventDefault(); // Evita la recarga de la página
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#' + modalId).modal('hide');
                updateDepartamentoTable(response.data); // Actualiza la tabla con la respuesta
                $('#success-message').html('Departamento actualizado exitosamente').show(); // Muestra el mensaje de éxito
    ...
    function updateDepartamentoTable(data) {
        let row = $('tr[data-id="' + data.id + '"]');
        if (row.length) {
            // Actualiza la fila existente
            row.find('td:nth-child(2)').text(data.nombre);
        } else {
            ...
            let newRow = `
                <tr data-id="${data.id}">
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-danger" onclick="deleteDepartamento(${data.id})">
                            Eliminar
                        </button>
                    </td>
                </tr>`;
